...
def estimate_mail(request, pk=None):
    ...
    notes += '</li></ul>'
    cost = hours * rate
    message = ''.join(['<h1 style="text-align: center">Statement of Work</h1><h2>%s hours of %s @ $%s/hour for %s = $%.2f</h2>' % (hours, estimate.subject, rate, estimate.client.name, cost), notes])
    if send_mail(
            request,
            'Estimate',
            message,
